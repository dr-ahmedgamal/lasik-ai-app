...
sph = st.number_input("Spherical (D)", format="%.2f", value=0.0)
cyl = st.number_input("Cylinder (D)", format="%.2f", value=0.0)
k1 = st.number_input("K1 (D)", format="%.2f", value=43.0)
k2 = st.number_input("K2 (D)", format="%.2f", value=44.0)
pachy = st.number_input("Corneal Thickness (µm)", value=520)
bcva = st.number_input("Pre-op BCVA (decimal)", format="%.2f", value=0.8)
...
def recommend_surgery():
    ...
    # Phakic IOL (Fallback for Age < 40)
    elif age < 40:
        return "Phakic IOL"

    # Pseudophakic IOL (Fallback for Age >= 40)
    else:
